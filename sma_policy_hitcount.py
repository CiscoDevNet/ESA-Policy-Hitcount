...
url_incoming = (
    f"http://{ESA_IP}:{ESA_PORT}/sma/api/v2.0/reporting/mail_policy_incoming/recipients_matched"
    f"?device_type=esa&startDate={startDate}&endDate={endDate}&top={TOP_N_POLICIES}"
)

url_outgoing = (
    f"http://{ESA_IP}:{ESA_PORT}/sma/api/v2.0/reporting/mail_policy_outgoing/recipients_matched"
    f"?device_type=esa&startDate={startDate}&endDate={endDate}&top={TOP_N_POLICIES}"
)
